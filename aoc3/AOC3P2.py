...
def count_trees(x_move, y_move, products):
    forest = get_biome()
    trees = traverse(x_move, y_move, forest)
    products.append(trees)
    return products


products = []  # List of trees hit per movement systems
x_move = 1  # Coordinates to move by
y_move = 1
products = count_trees(x_move, y_move, products)
